...
import 'package:lstracker/qr/qr_scanner_screen.dart';
...
class _DepositlisteState extends State<Depositliste> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(
          'Listes des échantillons CV à déposer',
          ...
          PopupMenuButton(
            ...
            onSelected: (value) {
              if (value == 'Deposer') {
                Navigator.push(
                  context,
                  MaterialPageRoute(builder: (context) => Acceptsampleform()),
                );
              } else if (value == 'Scan') {
                Navigator.push(
                  context,
      ...
      body: Column(
        children: [
          ...
          Text("Echantillons CV à déposer"),
        ],
      ),
    );
  }
